...
- Dedicated-tool enforcement for `issues/{n}/issue-field-values` and `PATCH repos/{owner}/{repo}/issues/{n}` in `check-api-tools.sh`, active when `block_api_tool_write` is enabled.
